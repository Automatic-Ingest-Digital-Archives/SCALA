...
|Task|Win10|
|----|-----|
|<span title="Trailing and leading whitespace in filenames causes RODA-In to crash in Windows."><i>Trim whitespace from filenames in Windows [optional]</i></span>|Manually trim whitespace from filenames.</br></br>OR run this bash script in a <b>Mac/Linux environment</b> in the root folder of your TS, before continuing on Windows.</br><code>find ~+ -type d &#124; while read dir; do cd "$dir"; echo "$dir"; $(for f in *; do mv "$f" $(echo "$f" &#124; xargs -0) > /dev/null 2>&1; done); done</code>|
